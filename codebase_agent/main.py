...
def analyze(
    ...
    try:
        ...
        with console.status("[bold green]Loading configuration..."):
            ...
            if missing_keys:
                console.print(
                    f"[red]Error: Missing configuration keys: {', '.join(missing_keys)}[/red]"
                )
                console.print(
                    "[yellow]Please check your .env file and configure the required variables.[/yellow]"
                )
                console.print(
                    "[cyan]Use 'codebase-agent setup <path>' to validate your configuration.[/cyan]"
                )
                sys.exit(1)
        ...
        console.print(
            "[yellow]Please check that the codebase path exists and is accessible.[/yellow]"
        )
        ...
        console.print(
            "[yellow]Please check that you have read access to the codebase directory.[/yellow]"
        )
    ...
    try:
        ...
        if not env_file.exists():
            ...
            if env_example.exists():
                ...
                console.print(
                    "[cyan]Copy .env.example to .env and configure your API settings.[/cyan]"
                )
            else:
        ...
        if missing_keys:
            ...
            for key in missing_keys:
                description = config_manager.REQUIRED_KEYS.get(
        ...
        if check_api and not missing_keys:
            ...
            try:
                with console.status("[bold green]Testing API connection..."):
                    ...
                    config_manager.get_model_client()
                    console.print("[green]✓ API configuration appears valid[/green]")

                    # Display configuration details (without sensitive info)
                    llm_config = config_manager.get_llm_config()
                    console.print(
                        f"[cyan]Model:[/cyan] {llm_config.get('model', 'Unknown')}"
                    )
                    console.print(
                        f"[cyan]Base URL:[/cyan] {llm_config.get('base_url', 'Unknown')}"
                    )

            except Exception as e:
                console.print(f"[red]✗ API configuration error: {e}[/red]")
                console.print(
                    "[yellow]Please check your API key and base URL settings.[/yellow]"
                )
                success = False
        ...
        if codebase_path.is_dir() and os.access(codebase_path, os.R_OK):
            ...
            console.print(
                f"[red]✗ Cannot read codebase directory: {codebase_path}[/red]"
            )
            success = False
        ...
        if success:
            ...
            console.print(
                f'[dim]  codebase-agent analyze {codebase_path} "your task description"[/dim]'
            )
        else:
            console.print("[bold red]✗ Setup validation failed[/bold red]")
            console.print(
                "[yellow]Please fix the issues above before running analysis.[/yellow]"
            )
            sys.exit(1)
